...
int main() {
	...
	while (!glfwWindowShouldClose(m_window)) {
		...
		glm::vec3 lightDir {0.f, -1.0f, 0.f};
		...
		phongShader.SetVector3("light.direction", lightDir);
		...
		for(int i = 0; i < boxCount; i++) {
			...
			litModel = glm::translate(litModel, glm::vec3((i - (boxCount / 2)) * 1.2f, 0.0f, 0.0f));
			litModel = glm::rotate(litModel, glm::radians(i * 15.f), glm::vec3(1.f, 0.0f, 0.0f));
