...
float sScene(vec3 p) {
    ...
    float pln0 = sPlane(p, vec3(0., 1., 0.), 6.);
    res = opUnionSmooth(res, pln0, 0.1);
...
vec3 dLighting(vec3 p) {
    ...
    float shadow = dSoftShadow(shadowOrigin, shadowDir, NEAR_PLANE, FAR_PLANE, 64.);
...
vec3 getColor(vec2 fragCoord) {
    ...
    vec3 sdfColor = dLighting(p) * (n * 0.5 + 0.5);
    vec3 bg = mix(vec3(0.2, 0.2, 0.8), vec3(1.), uv.y);
    return mix(bg, sdfColor, dDraw(dist));
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // Naive implementation of antialiasing. Samples neighbouring pixels
    // and takes the average color between the current pixel and its neighbours.  
    for(float aaY = 0.; aaY < aaSize; aaY++) {
        for(float aaX = 0.; aaX < aaSize; aaX++) {
            color += getColor(fragCoord + vec2(aaX, aaY) / aaSize);
            count += 1.;
        }
    }
